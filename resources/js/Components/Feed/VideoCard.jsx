...
import {
  ...
  RiDownload2Line, RiFlag2Line, RiArrowLeftLine, RiSearchLine,
} from 'react-icons/ri'
...
function ShareSheet({ videoUrl, videoTitle, onClose, onDownload, dlState, onReport }) {
...
useEffect(() => {
  if (showReportVideo) {
    ...
    setPlaying(false)
    // Lock scroll on the feed
    document.body.style.overflow = 'hidden'
  } else {
    // Restore scroll
    document.body.style.overflow = ''
    // Resume if this card is active
    if (isActive) {
 ...
 useEffect(() => {
  ...
  <MoreSheet
    ...
    onReport={() => setShowReportVideo(true)}
    videoRef={videoRef}
  />
)}
      ...
      <ReportVideoModal
         video={video}
         onClose={() => setShowReportVideo(false)}
       />
     )}
