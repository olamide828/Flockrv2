...
/**
 * Share sheet for a user's public profile link.
 * Mirrors ShareRoomSheet's pattern, minus the invite-code block
 * (profiles aren't private).
 */
export default function ShareProfileSheet({ user, onClose }) {
  ...
  const opts = [
    ...
    ...(canShare ? [{ label: 'More', Icon: RiShareForwardLine, color: '#FF6B35', bg: 'rgba(255,107,53,0.12)', href: null, onClick: () => navigator.share({ title: `${user.name} on Flockr`, url }).catch(() => {}) }] : []),
  ]

  return (
    <>
      <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 970, background: 'rgba(0,0,0,0.5)' }} />
      <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 971, background: 'rgba(18,18,18,0.98)', backdropFilter: 'blur(24px)', borderRadius: '20px 20px 0 0', borderTop: '1px solid rgba(255,255,255,0.08)', animation: 'shareProfileSlideUp 0.28s cubic-bezier(0.32,0.72,0,1)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 16px 14px' }}>
          <span style={{ color: '#fff', fontWeight: 700, fontSize: 15 }}>Share Profile</span>
      ...
      <style>{`@keyframes shareProfileSlideUp { from { transform: translateY(100%); } to { transform: translateY(0); } }`}</style>
    </>
  )
}