            ...
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                  ...
                  <button onClick={() => setQty(q => Math.min(selected.stock_quantity ?? 99, q + 1))} style={{ width: 30, height: 30, borderRadius: '50%', background: 'rgba(255,255,255,0.08)', border: 'none', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}><RiAddLine size={15} /></button>
                </div>
              </div>

              {error && <p style={{ color: '#EF4444', fontSize: 12, marginBottom: 12 }}>{error}</p>}

              <button onClick={handleConfirm} disabled={adding} style={{ width: '100%', padding: 15, borderRadius: 999, background: '#FF6B35', border: 'none', color: '#fff', fontSize: 14, fontWeight: 800, cursor: adding ? 'not-allowed' : 'pointer', opacity: adding ? 0.7 : 1 }}>
