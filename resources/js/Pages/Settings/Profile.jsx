        ...
        <div style={{ background: 'var(--flockr-card)', border: `1.5px solid ${address.is_default ? 'rgba(255,107,53,0.3)' : 'rgba(255,255,255,0.07)'}`, borderRadius: 16, padding: '14px 16px' }}>
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <p style={{ margin: '2px 0 0', color: 'rgba(255,255,255,0.45)', fontSize: 12 }}>{address.street}, {address.city}, {address.state}</p>
                </div>
            </div>
...
// ── Address form ──────────────────────────────────────────────────────────────
function AddressForm({ editing, onSaved, onCancel }) {
    ...
    return (
        <div style={{ background: 'var(--flockr-card)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 16, padding: '18px 16px', display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                    {['Home','Work','Other'].map(l => (
                        <button key={l} onClick={() => set('label', l)} style={{ flex: 1, padding: '8px 0', borderRadius: 10, background: form.label === l ? 'rgba(255,107,53,0.1)' : 'rgba(255,255,255,0.04)', border: `1px solid ${form.label === l ? 'rgba(255,107,53,0.35)' : 'rgba(255,255,255,0.08)'}`, color: form.label === l ? '#FF6B35' : 'rgba(255,255,255,0.5)', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
                            {l}
                        </button>
                    ))}
                </div>
            ...
            <FField label="Recipient Name *"><input value={form.recipient_name} onChange={e => set('recipient_name', e.target.value)} placeholder="Full name" style={inpSt} /></FField>
            <FField label="Phone *"><input value={form.phone} onChange={e => set('phone', e.target.value)} placeholder="080xxxxxxxx" type="tel" style={inpSt} /></FField>
            <FField label="Street Address *"><input value={form.street} onChange={e => set('street', e.target.value)} placeholder="House number, street name" style={inpSt} /></FField>

            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                <FField label="City *"><input value={form.city} onChange={e => set('city', e.target.value)} placeholder="e.g. Ikeja" style={inpSt} /></FField>
                <FField label="State *">
                    <select value={form.state} onChange={e => set('state', e.target.value)} style={{ ...inpSt, appearance: 'none' }}>
            ...
            <button onClick={handleSave} disabled={saving} style={{ padding: '12px', background: '#FF6B35', border: 'none', borderRadius: 12, color: '#fff', fontSize: 14, fontWeight: 700, cursor: saving ? 'not-allowed' : 'pointer', opacity: saving ? 0.7 : 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
...
export default function ProfileSettings({ banks = [], addresses: initialAddresses = [], has_pickup_address = false }) {
    ...
    const submitProfile = (e) => {
        e.preventDefault();
        post('/settings/profile', { forceFormData: true, preserveScroll: true, onSuccess: () => setEditing(false) });
    };
    ...
    const savePickupAddress = async () => {
        if (!pickupForm.pickup_street || !pickupForm.pickup_city || !pickupForm.pickup_state) {
            setPickupError('Street, city and state are required.');
            return;
        }
        setSavingPickup(true);
        setPickupError('');
        try {
            await axios.post('/api/seller/pickup-address', pickupForm);
            setPickupSuccess(true);
            setTimeout(() => setPickupSuccess(false), 3000);
        } catch (err) {
            setPickupError(err.response?.data?.message ?? 'Failed to save pickup address.');
        } finally {
        ...
        <>
            ...
            <div className="scroll-hidden h-screen overflow-y-auto" style={{ background: 'var(--flockr-black)' }}>
                ...
                <div style={{ position: 'sticky', top: 0, zIndex: 30, width: '100%', boxSizing: 'border-box', background: 'rgba(10,10,10,0.92)', backdropFilter: 'blur(20px)', borderBottom: '1px solid rgba(255,255,255,0.07)', padding: '14px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    ...
                    {tab === 'profile' && editing && (
                        <div style={{ display: 'flex', gap: 8 }}>
                            <button onClick={cancelEdit} style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', color: 'var(--flockr-muted)', borderRadius: 999, padding: '7px 16px', fontSize: 13, fontWeight: 500, cursor: 'pointer' }}>Cancel</button>
                ...
                <div style={{ maxWidth: 560, margin: '0 auto', padding: '0 0 100px' }}>
                    ...
                    {tab === 'profile' && (
                        <div>
                            {recentlySuccessful && (
                                <div style={{ margin: '16px 16px 0', background: 'rgba(0,217,126,0.1)', border: '1px solid rgba(0,217,126,0.3)', borderRadius: 12, padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 10 }}>
                                    <RiCheckLine size={16} color="var(--flockr-green)" />
                            ...
                            {isSeller && (
                                <div style={{ background: 'var(--flockr-card)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 16, overflow: 'hidden' }}>
                                    <div style={{ padding: '14px 16px', borderBottom: '1px solid rgba(255,255,255,0.07)', display: 'flex', alignItems: 'center', gap: 10 }}>
                                        ...
                                        <p style={{ color: 'var(--flockr-text)', fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 14, margin: 0 }}>Seller Pickup Address</p>
                                    </div>
                                    <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
                                        <p style={{ margin: 0, color: 'var(--flockr-muted)', fontSize: 13, lineHeight: 1.5 }}>
                                            This is where couriers will come to collect your packages. Keep it accurate so riders can find you.
                                        </p>
                                        <FormField label="Street Address">
                                            <input value={pickupForm.pickup_street} onChange={e => setPickupForm(p => ({ ...p, pickup_street: e.target.value }))} placeholder="House number, street name" style={inputStyle} />
                                        </FormField>
                                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                                            <FormField label="City">
                                                <input value={pickupForm.pickup_city} onChange={e => setPickupForm(p => ({ ...p, pickup_city: e.target.value }))} placeholder="e.g. Ikeja" style={inputStyle} />
                                            </FormField>
                                            <FormField label="State">
                                                <select value={pickupForm.pickup_state} onChange={e => setPickupForm(p => ({ ...p, pickup_state: e.target.value }))} style={{ ...inputStyle, appearance: 'none' }}>
                                                    <option value="">Select</option>
                                                    {NG_STATES.map(s => <option key={s} value={s}>{s}</option>)}
                                                </select>
                                            </FormField>
                                        ...
                                        {pickupError   && <p style={{ color: '#EF4444', fontSize: 12, margin: 0 }}>{pickupError}</p>}
                                        {pickupSuccess  && <p style={{ color: '#10B981', fontSize: 12, margin: 0 }}>✓ Pickup address saved!</p>}

                                        <button onClick={savePickupAddress} disabled={savingPickup} style={{ ...primaryBtnStyle, opacity: savingPickup ? 0.7 : 1 }}>
                    ...
                    {tab === 'security' && (
                        <div style={{ padding: 16 }}>
                            ...
                            <SectionCard title="Danger Zone">
                                ...
                                <button onClick={async () => { if (!confirm('Are you sure? This permanently deletes your account.')) return; try { await axios.delete('/api/users/me'); window.location.href = '/'; } catch { alert('Failed to delete account.'); } }} style={{ width: '100%', padding: '13px', borderRadius: 999, background: 'rgba(255,59,92,0.1)', border: '1px solid rgba(255,59,92,0.3)', color: 'var(--flockr-red)', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>Delete Account</button>
                            </SectionCard>
                        </div>
                    )}
                            ...
                            {auth?.user?.paystack_subaccount_code && !editingBank ? (
                                <>
                                    ...
                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                                        ...
                                        <button onClick={async () => { if (!confirm('Remove your payout account?')) return; try { await axios.delete('/settings/bank'); window.location.reload(); } catch { alert('Failed to remove.'); } }} style={{ height: 52, borderRadius: 16, border: '1px solid rgba(255,59,92,0.25)', background: 'rgba(255,59,92,0.08)', color: 'var(--flockr-red)', fontWeight: 600, fontSize: 14, cursor: 'pointer' }}>Remove</button>
                                    </div>
                                    <div style={{ background: 'rgba(0,217,126,0.08)', border: '1px solid rgba(0,217,126,0.18)', borderRadius: 16, padding: 16 }}>
                                        <p style={{ color: 'var(--flockr-green)', fontSize: 13, margin: 0, lineHeight: 1.6 }}>Your payouts are enabled and will be transferred automatically after successful order delivery.</p>
                                    </div>
                                </>
                            ) : (
                                <>
...
const labelSt     = { color: 'rgba(255,255,255,0.4)', fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', margin: '0 0 7px' };
...
const inpSt = { width: '100%', padding: '12px 13px', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, color: '#fff', fontSize: 14, outline: 'none', boxSizing: 'border-box', fontFamily: 'inherit' };
