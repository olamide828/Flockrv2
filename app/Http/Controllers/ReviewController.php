    ...
    public function store(Request $request, Order $order): JsonResponse
{
    ...
    // Resolve product_id BEFORE the transaction — needed on the review row itself
    $orderItem = $order->items()->first();
    $productId = $orderItem?->product_id;

    [$review, $coupon] = DB::transaction(function () use ($order, $rating, $validated, $photoKeys, $orderItem, $productId) {
        $review = Review::create([
            'order_id'   => $order->id,
            'buyer_id'   => $order->buyer_id,
            'seller_id'  => $order->seller_id,
            'product_id' => $productId,          // ← the actual fix
            'rating'     => $rating,
        ...
        DB::statement("
            UPDATE users SET
                avg_rating = (
                    SELECT COALESCE(ROUND(AVG(r.rating)::numeric, 2), 0.00)
                    FROM reviews r WHERE r.seller_id = ?
                ),
                total_reviews = (
                    SELECT COUNT(*) FROM reviews r WHERE r.seller_id = ?
                )
            WHERE id = ?
